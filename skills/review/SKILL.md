...
5. Decide which findings go to your partner.
   ...
   - Every blocker and major finding, and any finding you want to reject, downgrade, or fix in more than one reasonable way, or whose fix changes the approved spec (in a spec review, the approved design): ask your partner, one at a time, most severe first. Offer options from fix as proposed, another fix, defer, won't fix and reject, recommended first, and end with `Reply with <options in bold>, or **pause**.` Record the question under the item's Question before you ask.
