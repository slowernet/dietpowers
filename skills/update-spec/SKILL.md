...
The spec is what the plan, the reviewers and prove-done check against, so when specified behavior changes, the spec changes with it, in the same commit. This is a small edit to an approved spec: it applies once your partner has chosen to continue to planning. A new goal or feature goes back to the `brainstorm` skill.
