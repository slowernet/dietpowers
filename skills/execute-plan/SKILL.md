...
5. When the repository differs from what the plan assumes, make the routine call yourself and record it in the commit message. Stop to ask only when the difference would change an interface, a spec requirement, or another task. When a change would alter behavior the spec describes, invoke the `update-spec` skill.
