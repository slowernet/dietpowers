...
7. Note the current commit as `FIX_BASE`. Fix every finding that holds, in order: things that break or are insecure, then simple fixes, then complex ones. Start each fix with a test that reproduces the finding and fails, then make it pass. Commit.
8. If the findings came from a review subagent, dispatch one re-review using `code-reviewer.md` from requesting-code-review over `FIX_BASE..HEAD`, scoped to those findings. Fix what it finds that holds, the same way. Dispatch no third review: report anything still open to your partner.
9. Answer correct feedback by stating the fix and where it landed. Skip the agreement and the thanks — the change is the answer.
10. If you pushed back and turned out to be wrong, say what you checked and what it showed, then fix it. No apology, no defence of the pushback.
