...
Behavior:
- Renumber: step 3 purpose questions (create the tracker here, as step 3 does today); step 4 research; step 5 remaining design questions (constraints, success criteria, allowed input values, failure and rerun behavior, with the existing rules: ask only questions that change the design, record routine calls as assumptions, say so if the request seems mistaken); steps 6 to 9 are old 5 to 8. Keep step 0.
...
- `trackers.md contains "Research: skipped"` — fails if resume does not know the research step.
