...
- `README.md does not contain the old append sentence` — exactly `grep -qF 'section in the spec or plan' README.md && fail "README still describes the Review notes append"`; fails if line 116's old wording survives. The new change-note wording must not contain that phrase.
Run `bash tests/skills/check-skills.sh`.
