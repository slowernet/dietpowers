...
- `code-reviewer.md`: fix-check mode still runs the full test suite once and reports failures as ISSUEs. The Severity Guide is unchanged.
...
Context: spec "Review (`skills/review/SKILL.md`)"; Global Constraints (threshold, dispatch cap, snapshot); References (`AGENTS.md` anatomy, review SKILL.md at 995ca81).
...
- Rewrite the bullet containing `One re-review checks only the fixes` and the change note `Runs one scoped re-review, then stops.` to the second-pass rule.
